...
from src.day5_pt2 import convert_to_table_and_instructions, convert_table_to_array, main, convert_instruction_str_to_int, CrateMover9000, CrateMover9001
...
def test_convert_to_table_and_instructions(inputs_dir):
    ...
    assert table == ['    [D]    \n', '[N] [C]    \n', '[Z] [M] [P]\n', ' 1   2   3 \n']
    assert list(instructions) == ['move 1 from 2 to 1\n',
                                  'move 3 from 1 to 3\n',
                                  'move 2 from 2 to 1\n',
                                  'move 1 from 1 to 2']
...
def test_convert_table_to_array():
    ...
    assert [list(r.queue) for r in table] == [["Z", 'N'],
                                              ['M', "C", "D"],
                                              ['P']]


def test_convert_instruction_str_to_int():
    instruction_strs = ['move 1 from 2 to 1', 'move 3 from 1 to 3', 'move 2 from 2 to 1', 'move 1 from 1 to 2', 'move 13 from 7 to 8']
    instructions = convert_instruction_str_to_int(instruction_strs)

    assert [list(i) for i in instructions] == [[1, 2, 1], [3, 1, 3], [2, 2, 1], [1, 1, 2], [13, 7, 8]]
...
def test_make_move_CM9000_example_1():
    ...
    assert [list(r.queue) for r in new_table] == [["Z", 'N', "D"],
                                                  ['M', "C"],
                                                  ['P']]
...
def test_make_move_CM9000_example_2():
    ...
    assert [list(r.queue) for r in new_table] == [["C", 'M', ],
                                                  [],
                                                  ['P', "D", 'N', "Z", ]]
...
def test_make_move_CM9001_example_2():
    instruction = [3, 1, 3]
    table = []
    for idx, row in enumerate([["Z", 'N', "D"],
                               ['M', "C"],
                               ['P']],
                              0):
        table.append(LifoQueue())
...
def test_make_move_CM9000_example_3():
    ...
    assert [list(r.queue) for r in new_table] == [["C", ],
                                                  ['M', ],
                                                  ['P', "D", 'N', "Z", ]]
...
def test_make_move_CM9001_example_3():
    ...
    assert [list(r.queue) for r in new_table] == [['M'],
                                                  ['C'],
                                                  ['P', 'D', 'N', 'Z']]
